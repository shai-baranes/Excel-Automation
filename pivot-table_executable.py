...
# pyinstaller --onefile pivot-table_executable.py
# 
# from: https://www.youtube.com/watch?v=PXMJ6FS7llk&t=7832s

import pandas as pd
import os
import sys

application_path = os.path.realpath(os.path.dirname(__name__))
source_file = os.path.join(application_path, 'supermarket_sales.xlsx')
...
# Create the Results folder if not exist yet
if not os.path.exists(os.path.join(application_path, 'Results')):
    os.makedirs(os.path.join(application_path, 'Results'))


target_file = os.path.join(application_path, f'./Results/report_{month}.xlsx')
...
# File Rename:
# {"keys": ["ctrl+shift+2"], "command": "side_bar_rename"}


# new file shortcut (quick creation):
# {"keys": ["ctrl+t"], "command": "side_bar_new_file2"}


# Quick file name search by regex:
# {"keys": ["f10"], "command": "side_bar_find_files_path_containing"}
